...
// Main function that initializes FFmpeg and processes the video
fn main() -> Result<(), ffmpeg::Error> {
    ...
    // Process the video, extracting frames at the specified interval
    process_video(&input_file, output_dir, interval)?;

    Ok(())
}

// Function to process the video and extract frames
fn process_video(input_file: &str, output_dir: &str, interval: usize) -> Result<(), ffmpeg::Error> {
...
fn save_frame(frame: &Video, index: usize, output_dir: &str) -> std::result::Result<(), std::io::Error> {
    let file_path = format!("{}/frame{}.ppm", output_dir, index);
...
fn create_output_directory(output_dir: &str) -> std::result::Result<(), std::io::Error> {
